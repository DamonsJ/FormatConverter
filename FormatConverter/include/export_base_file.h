...
#include "..\include\export_3ds_dump.h"
	...
	namespace tri {
		namespace io {
			...
			*/
			...
			private:
				enum KnownTypes {
					KT_UNKNOWN, KT_PLY, KT_STL, KT_DXF, KT_OFF, KT_OBJ, KT_3DS, KT_VMI
				};
				static int &LastType()
				{
			...
			public:
				...
				static int Save(OpenMeshType &m, const char *filename, const int mask, CallBackPos *cb = 0)
				{
					...
					{
						err = Exporter3DS<OpenMeshType>::Save(m, filename, mask, cb);
						LastType() = KT_3DS;
					}
					else if (FileExtension(filename, "vmi"))
					{
						err = ExporterVMI<OpenMeshType>::Save(m, filename);
						LastType() = KT_VMI;
					}
					else {
						err = 1;
				...
				static const char *ErrorMsg(int error)
				{
					...
					case KT_3DS: return Exporter3DS<OpenMeshType>::ErrorMsg(error); break;
					case KT_VMI: return ExporterVMI<OpenMeshType>::ErrorMsg(error); break;
